...
このリポジトリは、thresholding、denoising、edge detection、classical segmentationを、synthetic ground truthと公開画像で比較する再現可能なコンピュータビジョン実験集です。

手法ごとの挙動と失敗条件を、定量評価と比較画像の両方から確認したいR&Dエンジニアに役立ちます。

主な技術的証拠は次のとおりです。

- 5つのregistered experimentと165件のmethod-condition evaluation
- synthetic ground truthと固定した6画像のlabeled public subset
- CSV metrics、比較図、6件のSHA-256 manifest対象
- 統一CLI、31テスト、Python 3.10〜3.14のCI

結果の解釈、再現手順、制約の詳細は英語本文を参照してください。
